...
export default function ReviewPanel({
  ...
  const getScoreColor = (score: number) => {
    if (score >= 0.7) return 'bg-green-50/80 text-green-700'
    if (score >= 0.5) return 'bg-amber-50/80 text-amber-700'
    return 'bg-muted/50 text-secondary'
  }

  if (submitted) {
    return (
      <div className="p-6 rounded-xl bg-green-50/80 text-green-700 border border-green-200 text-center">
        <span className="text-green-600 font-medium">审核已提交</span>
      </div>
    )
  }
        ...
        {drugs.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {drugs.slice(0, 4).map((drug, idx) => (
              <span
                key={idx}
                className="px-2 py-0.5 text-xs rounded-md bg-primary/8 text-primary"
              >
                {drug.drugName}
              </span>
            ))}
      ...
      <div className="p-4 rounded-xl bg-background border border-border">
        ...
        {decision === 'modify' && (
          <div className="mb-3">
            ...
            <select
              ...
              className="w-full p-2.5 rounded-lg bg-surface-inset border border-[rgba(155,175,200,0.15)] text-sm text-foreground focus:outline-none focus:border-primary/40"
            >
        ...
        {(decision === 'modify' || decision === 'reject') && (
          <div className="mb-3">
            ...
            <textarea
              ...
              placeholder="请输入审核意见..."
              rows={2}
              className="w-full p-2.5 rounded-lg bg-surface-inset border border-[rgba(155,175,200,0.15)] text-sm text-foreground resize-none focus:outline-none focus:border-primary/40 placeholder:text-muted-foreground/50"
            />
          </div>
        )}
        ...
        {decision && (
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full p-2.5 rounded-lg bg-surface-inset border border-[rgba(155,175,200,0.15)] text-sm text-foreground mb-2 focus:outline-none focus:border-primary/40"
            >
        ...
        {decision && (
          <button
            ...
            className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-semibold text-sm transition-all duration-150 hover:bg-primary/90"
          >
            提交审核
          </button>
        )}
