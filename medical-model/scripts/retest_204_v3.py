...
Uses model service direct API with proper budget reset (query params)."""
import json
import requests
import time
import re
...
OUTPUT_PATH = "verification_results_v2/retest_204_after_fix_v3.json"
...
def reset_budget():
    try:
        resp = requests.post(BUDGET_RESET_URL, params={"userId": "default"}, timeout=10)
        time.sleep(0.5)
    except Exception:
        pass
...
def main():
    ...
    for i, disease in enumerate(diseases):
        ...
        if (i + 1) % 20 == 0:
            ...
            print(f"  Saved intermediate results ({i+1} diseases)")
