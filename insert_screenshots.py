"""
Insert system page screenshots into thesis docx.
Strategy: Add content at end of document, then move XML elements to correct position.
"""
import sys
...
output_path = 'grad_doc/初稿(2).docx'
...
pages = [
    ...
    },
    {
        ...
        'image': 'privacy-config.png',
        'fig_num': '5-5'
    },
    {
        'name': '隐私效果可视化页面',
        'desc': '隐私效果可视化页面提供差分隐私保护效果的直观展示，帮助用户理解隐私机制对推荐结果的影响。页面包含多个可视化图表：隐私预算消耗趋势图展示隐私预算随推荐次数的消耗变化；噪声分布对比图展示拉普拉斯噪声和高斯噪声的概率密度分布差异，直观说明两种噪声机制的特点；推荐结果对比图展示有无噪声注入时推荐得分的差异，体现差分隐私对推荐排序的影响；效用-隐私权衡曲线展示不同ε值下推荐准确率的变化趋势。用户可通过交互控件调整参数，实时观察隐私保护效果的变化。界面如图5-6所示。',
...
# Step 1: Add all new paragraphs at the end of the document body using python-docx API
# This ensures proper image handling

body = doc.element.body
# Find the paragraph element for section 5.3 content (paragraph index 207)
target_para = doc.paragraphs[207]
target_element = target_para._element

# Collect the XML elements we create
new_elements = []
...
for page_info in pages:
    ...
    pf.first_line_indent = Cm(0.74)  # ~2 chars indent
    pf.line_spacing = 1.5
    pf.space_after = Pt(6)
...
# Step 2: Move all new elements from end of body to after paragraph [207]
# They were added at the end, now we need to move them right after target_element

for elem in new_elements:
    # Remove from current position (end of body)
    body.remove(elem)

# Now insert them after target_element
...
print(f'Added {len(pages)} page screenshots with descriptions')
...
for para in doc2.paragraphs:
    ...
    if drawings:
        img_count += 1
print(f'Images verified in output: {img_count}')